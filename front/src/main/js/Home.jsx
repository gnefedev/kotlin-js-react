...
class Home extends React.Component {
    state = {
        loaded: false,
        color: searchAsMap(this.props.location.search)["color"],
        brand: searchAsMap(this.props.location.search)["brand"]
    };
            ...
            <Layout>
                <Header>
                    <HomeHeader
                        ...
                        onBrandChange={brand => this.navigateToChanged({brand})}
                        colors={this.state.colors}
                        color={this.state.color}
                        onColorChange={color => this.navigateToChanged({color})}
                    />
                </Header>
                <Content>
    ...
    async componentDidMount() {
        this.props.history.listen(location => {
            let query = searchAsMap(location.search);
            this.setState({
                brand: query["brand"],
                color: query["color"]
            });
...
const HomeHeader = (
    ...
    <div>
        ...
        <Dropdown
            ...
            options={withDefault("all", brands.map(value => ({label: value, value: value})))}/>
        Color:
        <Dropdown
            value={color}
            onChange={e => onColorChange(e.value)}
            options={withDefault("all", colors.map(value => ({label: value, value: value})))}/>
    </div>
);
...
const HomeContent = ({cars}) => (
    <DataTable value={cars}>
        <Column header="Brand" body={rowData => rowData["brand"]}/>
        <Column header="Color" body={rowData => <span style={{color: rowData['color']}}>{rowData['color']}</span>}/>
        <Column header="Year" body={rowData => rowData["year"]}/>
    </DataTable>
);
...
function withDefault(label, options) {
    options.unshift({label: label, value: null});
    return options;
}
